...
int main()
{
	...
	for ( std::size_t i_x0=0; i_x0<intervalNumber; ++i_x0)	// computing the estimated values of the integral of f(x)...
	{
		for ( std::size_t i_x1=0; i_x1<intervalNumber; ++i_x1)
		{
			...
			MultiDimGrid::DoubleArray<2> weights1 = gridFunc1.integration_weights(g_i);	// ...extracting the respective integration weights in test case 1,
			MultiDimGrid::DoubleArray<2> weights2 = gridFunc2.integration_weights(g_i);	// ...test case 2 and...
			MultiDimGrid::DoubleArray<2> weights3 = gridFunc3.integration_weights(g_i);	// ...test case 3,...
			
			integral1 += gridFunc1.value(g_i) * weights1[0]*weights1[1];	// ...and summing up the function values multiplied by the corresponding overall weight in test case 1,...
			integral2 += gridFunc2.value(g_i) * weights2[0]*weights2[1];	// ...test case 2 and...
			integral3 += gridFunc3.value(g_i) * weights3[0]*weights3[1];	// ...test case 3
		}
	}
